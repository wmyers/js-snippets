function subStringFinder(str, subStr){
  ...
  var idx = 0, //the starting index of the subStr in str, if it exists
      ...
      ii = 0, //iteration index that runs thru str. This does not always increment linearly but will jump back and then continue
      len = str.length,
   ...
   while(true){
      ...
      if(str[ii] == subStr[j]){
         ...
         //check if a non-starting char (j > 0) is a potential starting char (startVal)
         //only do this once per subsStr check (indx_off == 0)
         ...
         //if the sequence doesn't match but a check has started on a previous index
         if(j > 0){
            //reset j
            j = 0;

            //we are starting again here, so check if you came across any potential starting char
            //that was not at the start of the previous subStr check. This value is set as indx_off,
            //apply this to the last starting point (idx) set that value to the iterator index (this makes it jump back)
            if(indx_off > 0){
               ii = idx  + indx_off;
            }

            //reset indx_off for the next subStr check
            indx_off = 0;
         }
      ...
      //need to update here because the next checks are synched to the next subStr check
      ii ++;

      //now check for a starting point or a match
      if(j == 0){
        //if a starting point, store the possibly correct return value,
        //this is now the current starting point
        idx = ii;
      }else if (j == subLen){
      ...
      //or check if you have gone out of range, in which case the subStr is not contained in str
      if(idx > lastIndx){
         return -1;
      }
  }
